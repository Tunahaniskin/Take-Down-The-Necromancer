...
export class Cards {
    ...
    canvas.addEventListener("click", (event) => {
      ...
      for (let i = 0; i < 5; i++) {
        ...
        if (clickX >= cardstartX && clickX <= cardendX && clickY >= cardstartY && clickY <= cardendY) {
          // İlk boş player kartını bul ve doldur
          for (let k = 0; k < 3; k++) {
            
              
                this.playerCards[k] = this.mainCards[i];
 ...
 drawCardsBar(ctx) {
  ...
  drawCardsBarIcon(ctx) {
   ...
   /* const canvas = document.getElementById("gameCanvas");
    canvas.addEventListener("click", (event) => {
     // player kartlarını oluşturma (tıklama ile)
     for(let k = 0 ; k<3 ; k++){
        let clickX = event.offsetX;  
        let clickY = event.offsetY;
        for(let i = 0 ; i < 6 ; i++){
          const cardstartX = this.MainstartX + i * (this.MaincardWidth + this.MaincardSpacing);
          const cardendX = this.MainstartX + i * (this.MaincardWidth + this.MaincardSpacing) + this.MaincardWidth;
          const cardstartY = this.MainstartY;
          const cardendY = this.MainstartY + this.MaincardHeight;
            if(clickX >= cardstartX && clickX <= cardendX && clickY >= cardstartY && clickY <= cardendY){
